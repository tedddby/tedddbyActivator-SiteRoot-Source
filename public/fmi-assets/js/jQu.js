...
const fetchSN = () => {
    fetch("/api/serials.adm", { method:"post" })
    .then(res => res.json())
    .then(data => {
        if(data.data){
            alert(data.data);
            ...
            if(data.length > 0){
                ...
                data.forEach(ele => {
                    ...
                    temp+="<td>"+ele.by_user+"</td>";
                    temp+=`<td><button class="btn btn-sm btn-neutral" type="button" onclick="DeleteSerial('${ele.id}')">Delete</button></td>`;
                    temp+=`<td><button class="btn btn-sm btn-neutral" type="button" onclick="DeleteRecord('${ele.serial}')">Clear</button></td>`;
                    temp+="</tr>";
                })
                serialList.innerHTML=temp;
                get("serial_count").innerHTML=data.length;
    ...
    if(id){
        ...
        .then(data => {
            if(data.status == "success"){
                alert("Serial Deleted!, It will disapper from the table after reload!");
            }else{
                alert("Failed!")
            }
        })
...
const DeleteRecord = (serial) => {
    if(serial){
        fetch("/api/deleteRecord.adm", {
            method:"post",
            headers:{
                "Content-Type":"application/json"
            },
        ...
        .then(data => {
            if(data.status == "success"){
                alert("Activation Record Cleared!");
            }else{
                alert("Failed!")
            }
        })
